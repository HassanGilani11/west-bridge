...
class Wpe_Admin_Ux {
	...
	public function plugin_install_admin() {
		...
		if ( $is_plugin_page && wpe_use_alternate_api() ) {
	...
	public function wpe_notices() {
		...
		foreach ( $notices as $notice ) {
			if ( $notice && ( $notice['isGlobal'] || in_array( get_current_screen()->id, $notice['screenIds'], true ) ) ) {
				$is_dismissible_class = isset( $notice['isDismissible'] ) && $notice['isDismissible'] ? ' is-dismissible' : '';
				?>
				<div class="notice notice-<?php echo esc_attr( $notice['type'] . $is_dismissible_class ); ?> " style="padding: 8px 12px;">
					<div>
						<?php echo wp_kses_post( $notice['message'] ); ?>
					</div>
					<?php
					if ( $notice['showWpeLogo'] ) {
						$this->wpe_logo_horizontal();
					}
					?>
				</div>
				<?php
